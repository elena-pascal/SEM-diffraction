...
class EdgeBeta(object):
    ...
    def uInf_x(self, x, y):
        return (self.b/(2.*pi)) * ( atan(y/x) + ( x*y/ \
            ( 2.*(1.-self.nu)*(x*x+(y*y)) ) ) )

    def uInf_y(self, x, y):
        return (-self.b/(2.*pi)) * ( (1.-(2.*self.nu))*log(x*x+ \
